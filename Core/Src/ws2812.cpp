...
void ws2812_LED::init_led_buffer() {
...
void ws2812_LED::write_color_bytes_to_buffer(uint8_t index, uint8_t green_byte, uint8_t red_byte, uint8_t blue_byte, uint32_t color_code) {
...
//	if (status == HAL_OK) {																			//Don't populate the buffer while the DMA transfer is going on.
		for (int g = 7; g >= 0; g--) {
			if (green_byte & mask) {
...
void ws2812_LED::set_led_color_by_index(uint8_t index, uint32_t color_code) {
...
void ws2812_LED::set_all_led_colors(uint32_t color_code) {
...
//Display the colors.
void ws2812_LED::render_led_colors() {
	HAL_TIMEx_PWMN_Start_DMA(&hal_timer, TIM_CHANNEL_2, (uint32_t*) out_buf, BUFF_SIZE);
}
